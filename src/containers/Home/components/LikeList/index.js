...
class LikeList extends Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <div className="likelist__header"></div>
        <div className="likelist__list">
          {data.map((item, index) => {
            return <LikeItem key={item.id} data={item}></LikeItem>;
          })}
        </div>
        {/* 根据加载次数判断最下方显示loading效果还是“查看更多” */}
        {pageCount < 3 ? (
          <Loading />
        ) : (
          <a
            href="https://m.dianping.com/tuan/beijing/list/173_0_10_0"
  ...
  handleScroll = () => {
    ...
    const likelistTop = this.divElem.offsetTop;
    const likeListHeight = this.divElem.offsetHeight;
